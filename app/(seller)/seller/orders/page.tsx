...
export default function SellerOrdersPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ...
    () => listOrders({ role: 'seller', ...filters }),
    {
  ...
  if (data?.data) {
    data.data.forEach((order, index) => {
      ...
      if (index % 3 === 0) {
        unreadCountByOrderId[order.id] = Math.floor(Math.random() * 5) + 1;
      }
    });
  }
    ...
    <div className="container mx-auto px-4 py-8">
      <main role="main">
        ...
        <div className="mt-8 text-sm text-gray-500">
          <p>Keyboard shortcuts: Press <kbd className="px-2 py-1 bg-gray-100 rounded">g</kbd> then <kbd className="px-2 py-1 bg-gray-100 rounded">o</kbd> to navigate to orders</p>
        </div>
      </main>
    </div>
  );
